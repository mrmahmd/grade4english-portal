(() => {
  ...
  const stations=[['vocab','01','Words','Listen, look & say','الكلمات'],['explain','02','Discover','Understand the lesson','الشرح'],['phonics','03','Sounds','Listen & repeat','الأصوات'],['reading','04','Read & listen','Enjoy the story','القراءة والاستماع'],['writing','05','Write','Make it your own','الكتابة'],['practice','06','Play & practise','Your lesson challenge','التدريبات']];
  ...
  const note=(en,ar)=>`<p class="helper">${esc(en)} <span lang="ar" dir="rtl">${esc(ar)}</span></p>`;
  ...
  function lessonPage(){
    ...
    let content='';
    if(st==='map'){content=`<section class="section-head"><div><span class="eyebrow">SIX SMALL STEPS</span><h2>Your lesson adventure</h2></div><p>Choose a station. Explore at your pace.</p></section><div class="station-grid">${stations.map(([id,num,title,sub,ar])=>`<button class="station-card tone-${id}" data-action="lesson" ${lessonAttrs(id)}><span class="station-num">${num}</span><span class="station-title">${title}</span><span>${sub}</span><small lang="ar" dir="rtl">${ar}</small><b>${id==='practice'?(passed(u.id,l.id)?'✓ Challenge passed':'25 questions →'):(progress.stations[`${key(u.id,l.id)}-${id}`]?'✓ Explored':'Explore →')}</b></button>`).join('')}</div>`;}
    if(st==='vocab')content=`<div class="section-head"><div><span class="eyebrow">LOOK. LISTEN. SAY.</span><h2>Your new words</h2></div><span class="tag">${l.vocab.length} words</span></div>${note('Tap Listen, then say the word aloud.','استمع ثم انطق الكلمة بصوت واضح.')}<div class="word-grid">${l.vocab.map(v=>`<article class="word-card">${wordArt(v[0])}<h3>${esc(v[0])}</h3><p>${esc(v[2].replace('___',v[0]))}</p>${audio(v[0])}</article>`).join('')}</div>${finishStation(st)}`;
    ...
    if(st==='phonics')content=`<section class="sound-intro"><span class="eyebrow">TUNE YOUR LISTENING EARS</span><h2>${esc(l.phonics.title)}</h2>${note('Listen to each word. Repeat it slowly. Notice the sound.','استمع للكلمة ورددها ببطء ولاحظ الصوت.')}<p class="small-print">The Listen buttons pronounce whole words.</p></section><div class="sound-grid">${l.phonics.words.map(w=>`<article><h3>${esc(w)}</h3>${audio(w,'Listen & repeat')}</article>`).join('')}</div>${finishStation(st)}`;
